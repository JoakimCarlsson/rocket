    ...
    <>
      ...
      <group position={[0, PLATFORM_HEIGHT, 0]}>
        <RocketModel
          ...
          tilt={rocket.tilt}
          onLanded={onPartLanded}
        />
        <Labels
          layout={layout}
          visible={labels && Math.abs(rocket.tilt) <= 10}
        />
      </group>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.02, 0]}>
        ...
        <meshStandardMaterial
          color="#0a0b0e"
          roughness={quality === "high" ? 0.35 : 0.9}
          metalness={0.6}
        />
      </mesh>
      {quality === "high" && (
        <ContactShadows
          position={[0, PLATFORM_HEIGHT + 0.02, 0]}
          scale={spread * 3}
          blur={2.4}
          opacity={0.75}
          far={layout.height}
      ...
      <Sparkles
        ...
        scale={[120, Math.max(60, layout.height), 120]}
...
function Platform({ radius }: { radius: number }) {
  ...
  return (
    <group>
      ...
      <mesh
        position={[0, PLATFORM_HEIGHT + 0.01, 0]}
  ...
  useFrame(({ clock }) => {
    if (light.current)
      light.current.color.setRGB(
        Math.sin(clock.elapsedTime * 2.5) > 0 ? 3 : 0.3,
        0.1,
        0.05,
      );
  });
...
export function BayCamera({
  ...
  const fit = useMemo(() => {
    ...
    const byWidth =
      horizontalExtent / 2 / Math.tan(fov / 2) / Math.min(1.4, aspect);
    const center = PLATFORM_HEIGHT + (layout.height + layout.minY) / 2;
    return {
      distance: Math.max(30, byHeight * 1.3, byWidth),
      targetY: center - verticalExtent * 0.12,
    };
  ...
  useFrame((_, delta) => {
    ...
    c.autoRotate =
      autoRotate && !interacting.current && now / 1000 - idleSince.current > 3;
    if (framing.current.active) {
      ...
      const distance = THREE.MathUtils.damp(
        offset.length(),
        fit.distance,
        2.6,
        delta,
