  ...
  const nativeShare = async () => {
    try {
      await navigator.share({
        title: rocket.name,
        text: `I built ${rocket.name} on ROCKET.JDADDY`,
        url,
      });
    } catch {
      /* dismissed */
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="panel w-full max-w-[480px] rounded-3xl p-6"
      >
        ...
        <div className="mt-4 flex gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-xl border border-line bg-black/40 px-3 py-2.5 font-mono text-[11px] text-muted outline-none"
          />
          <button
            onClick={copy}
            className="flex items-center gap-2 rounded-xl bg-text px-4 font-mono text-[11px] tracking-[0.14em] text-black"
          >
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          ...
          {typeof navigator !== "undefined" && "share" in navigator && (
            <button
              onClick={nativeShare}
              className="col-span-2 flex h-11 items-center justify-center gap-2 rounded-xl border border-line font-mono text-[10.5px] tracking-[0.14em] text-muted hover:text-text"
            >
