...
export function TransitionOverlay({
  ...
  return (
    <motion.div
      className="fixed inset-0 z-40 grid place-items-center overflow-hidden bg-black"
    ...
    >
      ...
      >
        <div className="label-xs mb-3 text-accent">
          Transferring to pad 39-X
        </div>
        <div className="font-display text-3xl font-extrabold sm:text-5xl">
          {name}
        </div>
...
const CUE_BANNERS: Partial<
  Record<LaunchCue, { text: string; tone: "ok" | "warn" | "bad" }>
> = {
...
export function LaunchHUD({
  ...
  cue: { value: LaunchCue; id: number } | null;
  telemetry: { current: Telemetry };
  plan: LaunchPlan;
  onSkip: () => void;
}) {
  const [readout, setReadout] = useState<Telemetry>({
    altitude: 0,
    speed: 0,
    t: -3,
  });
    ...
    <div className="pointer-events-none fixed inset-0 z-30">
      <AnimatePresence>
        {count && (
          ...
          >
            <span className="font-display text-[28vw] leading-none font-extrabold text-white/90 [text-shadow:0_0_80px_rgba(255,91,31,0.5)] sm:text-[180px]">
              {count}
            </span>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {banner && cue && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="absolute top-[18%] right-0 left-0 text-center"
          >
            <span
              className={`inline-block border-y px-6 py-2 font-mono text-[13px] tracking-[0.35em] backdrop-blur-sm sm:text-[15px] ${banner.tone === "bad" ? "border-bad/50 bg-bad/15 text-bad" : banner.tone === "warn" ? "border-warn/40 bg-warn/10 text-warn" : "border-white/20 bg-black/30 text-text"}`}
            >
              {arrived ?? banner.text}
            </span>
          </motion.div>
      ...
      <button
        onClick={onSkip}
        className="pointer-events-auto absolute right-4 bottom-6 rounded-lg border border-white/15 bg-black/40 px-3 py-2 font-mono text-[10px] tracking-[0.18em] text-muted backdrop-blur hover:text-text sm:right-8"
      >
        SKIP TO REPORT
      </button>
    </div>
      ...
      >
        <div className="border-b border-line px-6 pt-6 pb-5">
          ...
          <motion.h2
            ...
            className={`mt-3 font-display text-[26px] leading-[1.05] font-extrabold ${tone}`}
          >
            {report.headline}
          </motion.h2>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 border-t border-line p-3">
          {failed ? (
            ...
            <button
              onClick={onShare}
              className="col-span-2 flex h-12 items-center justify-center gap-2 rounded-xl bg-accent font-display text-[13px] font-bold tracking-[0.1em] text-black"
            >
          ...
          {failed ? (
            <ReportButton icon="share" label="SHARE" onClick={onShare} />
          ) : (
            <ReportButton
              icon="wrench"
              label="REPAIR WITH AI"
              onClick={onRepair}
            />
          )}
        </div>
...
function ReportButton({
  ...
  return (
    <button
      onClick={onClick}
      className="flex h-11 items-center justify-center gap-2 rounded-xl border border-line font-mono text-[10.5px] tracking-[0.14em] text-muted transition hover:border-line-strong hover:text-text"
    >
      ...
      <AnimatePresence>
        {current && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: -20 }}
            className="panel flex items-center gap-3 rounded-2xl border-accent/40 py-2.5 pr-5 pl-3"
          >
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-accent font-display text-lg font-extrabold text-black">
              ★
            </span>
            <div>
