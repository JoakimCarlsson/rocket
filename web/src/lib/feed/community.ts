...
import { type LaunchPlan, simulateLaunch } from "../sim/simulate";
...
const SEEDED: Seeded[] = [
  {
    slug: "tax-write-off",
    name: "THE TAX WRITE-OFF",
    creator: "@cfo_of_space",
    prompts: ["Make the cheapest rocket possible", "add a flag on top"],
    likes: 4211,
    base: "starter",
  },
  {
    slug: "lunar-shopping-cart",
    name: "LUNAR SHOPPING CART",
    creator: "@moonmoth",
    prompts: [
      "Build me a lunar rocket with three boosters",
      "add cargo",
      "paint it silver with red stripes",
    ],
    likes: 3890,
    base: "starter",
  },
  {
    slug: "absolute-unit-mk-iv",
    name: "ABSOLUTE UNIT MK IV",
    creator: "@thrustlord",
    prompts: [
      "make it enormous",
      "add 12 boosters",
  ...
  {
    slug: "the-problem-solver",
    name: "THE PROBLEM SOLVER",
    creator: "@chaos_director",
    prompts: [
      "Make me the dumbest rocket imaginable with 12 boosters",
      "add googly eyes",
    ],
    likes: 12044,
    base: "starter",
  ...
  {
    slug: "duck-of-destiny",
    name: "DUCK OF DESTINY",
    creator: "@duckmission",
    prompts: [
      "put a rubber duck on top",
      "paint it yellow",
  ...
  {
    slug: "sideways-situation",
    name: "THE SIDEWAYS SITUATION",
    creator: "@gravitydenier",
    prompts: ["make it sideways", "add wings", "add six boosters"],
    likes: 2877,
    base: 88,
  },
  {
    slug: "red-planet-rental",
    name: "RED PLANET RENTAL",
    creator: "@dustydreamer",
    prompts: [
      "Build a Mars rocket",
      "make it taller",
      "paint it white with red bands",
    ],
    likes: 5310,
    base: "starter",
  },
  {
    slug: "chrome-cathedral",
    name: "CHROME CATHEDRAL",
    creator: "@shinyobjects",
    prompts: [
      "make it chrome",
      "make it enormous",
      "add a spike on top",
  ...
  },
  {
    slug: "the-intern",
    name: "THE INTERN",
    creator: "@nasa_intern_ish",
    prompts: ["make it tiny", "add a propeller", "add googly eyes"],
    likes: 7011,
    base: 12,
  },
  {
    ...
    creator: "@lil_payload",
    prompts: [
      "add a habitat with 12 passengers",
      "make it wider",
      "paint it yellow and black",
    ],
    likes: 3140,
    base: "starter",
  },
  {
    slug: "checkerboard-regret",
    name: "CHECKERBOARD REGRET",
    creator: "@stage_fright",
    prompts: [
      "checkered black and white",
      "add four more boosters",
      "add grid fins",
    ],
    likes: 1996,
    base: 77,
  },
  {
    ...
    name: "SOLAR REGRET",
    creator: "@apogee_andy",
    prompts: ["send it to the sun", "add spikes", "make it pointier"],
    likes: 2603,
    base: 3131,
  },
];
...
function generatedPost(index: number): FeedPost {
  ...
  const rng = createRng(seed);
  const prompts = Array.from({ length: randInt(rng, 1, 3) }, () =>
    pick(rng, PROMPT_POOL),
  );
  const rocket = withIdSource(createRng(seed ^ 0x5bd1e995), () => {
  ...
  return {
    ...
    prompt: prompts.join(" → "),
    rocket,
    likes: Math.floor(rng() ** 3 * 9000) + randInt(rng, 3, 90),
    plan: simulateLaunch(rocket, 1),
  };
}
...
export function feedPage(page: number, size = 12): FeedPost[] {
  ...
  for (let i = page * size; i < (page + 1) * size; i++) {
    const id =
      i < SEEDED.length ? `c-${SEEDED[i].slug}` : `g-${i - SEEDED.length}`;
