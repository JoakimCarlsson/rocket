...
const size = z.preprocess(sizeFromNumber, z.enum(["tiny", "small", "medium", "large", "huge"]));
...
export const actionSchema = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("set_destination"), value: z.enum(["orbit", "moon", "mars", "sun", "nowhere"]) }),
  z.object({ type: z.literal("add_boosters"), count: count.min(1), size: size.optional(), color: color.optional() }),
  z.object({ type: z.literal("remove_boosters"), count: count.optional(), ids: z.array(componentId).max(24).optional() }),
  z.object({ type: z.literal("set_booster_count"), count, size: size.optional() }),
  z.object({
    type: z.literal("scale"),
    target: z.enum(["rocket", "stages", "boosters", "payload", "engines", "fins", "decor", "id"]),
  ...
  z.object({ type: z.literal("remove_stage"), id: componentId.optional(), position: z.enum(["top", "bottom"]).optional() }),
  z.object({
  ...
  z.object({ type: z.literal("set_finish"), value: z.enum(["matte", "satin", "metallic", "chrome", "glossy"]) }),
  z.object({ type: z.literal("set_pattern"), value: z.enum(["solid", "stripes", "bands", "checker", "split"]) }),
  z.object({
  ...
  z.object({
    ...
    kind: z.enum(["capsule", "fairing", "satellite", "cargo", "habitat", "none"]).optional(),
  ...
  z.object({ type: z.literal("set_legs"), count: z.number().int().min(3).max(8).optional(), size: size.optional() }),
  ...
  z.object({ type: z.literal("remove_decor"), id: componentId.optional(), kind: decorKind.optional() }),
  z.object({ type: z.literal("set_tilt"), degrees: z.number().min(-180).max(180) }),
