...
import { chance, pick, type Rng, randInt } from "../rocket/random";
...
function numberBefore(text: string, nouns: string): number | null {
  const match = text.match(
    new RegExp(`\\b${NUMBER_PATTERN}\\s+(?:[a-z-]+\\s+){0,2}?(?:${nouns})`),
  );
...
function sizeNear(text: string, nouns: string): SizeClass | undefined {
  const match = text.match(
    new RegExp(
      `\\b(tiny|small|little|mini|big|bigger|large|larger|huge|giant|enormous|massive|colossal|gigantic|absurd|ridiculous|stupidly big)\\s+(?:[a-z-]+\\s+)?(?:${nouns})`,
    ),
  );
...
function colorsIn(text: string): string[] {
  ...
  for (const name of names) {
    ...
    while ((m = re.exec(text))) {
      const index = m.index;
      if (!found.some((f) => Math.abs(f.index - index) < 2))
        found.push({ index, value: name });
    }
  }
...
export function interpretLocally(
  ...
  if (options.mode === "repair")
    return repairLocally(rocket, rng, options.mission);
  ...
  if (has(text, /\bsurprise\b|\bwhatever\b|\bdealers choice\b|\bgo wild\b/))
    surprise(ctx);
  if (
    has(
      text,
      /\b(cheap|cheapest|budget|inexpensive|affordable|low cost|broke)\b/,
    )
  )
    cheap(ctx);
  destinations(ctx);
  ...
  if (
    has(
      text,
      /\b(stupid|dumb|dumbest|stupidest|ridiculous|absurd|insane|unhinged|chaotic|chaos|silly|cursed|nonsense|goofy|deranged|weirder|weird)\b/,
    )
  )
    chaos(ctx);
  if (
    has(
      text,
      /\b(sleek|elegant|beautiful|clean|serious|professional|classy|tasteful)\b/,
    )
  )
    sleek(ctx);
  ...
  const rename = instruction.match(
    /(?:call it|name it|rename(?: it)?(?: to)?|named)\s+["']?([^"'.!?,]{2,40})/i,
  );
  if (rename)
    emit(ctx, `Registered as ${rename[1].trim().toUpperCase()}.`, {
      type: "rename",
      name: rename[1].trim(),
    });
  ...
  if (after.chaos - before.chaos >= 18 && chance(rng, 0.8))
    fragments.push(pick(rng, CHAOS_QUIPS));
  else if (after.reliability < 30 && chance(rng, 0.5))
    fragments.push(pick(rng, LOW_RELIABILITY_QUIPS));
  fragments.push(...result.notes.slice(0, 1));

  const name =
    rocket.name === DEFAULT_NAME && !rename
      ? generateName(result.config, rng)
      : undefined;
...
function destinations(ctx: Context): void {
  ...
  const building = has(
    text,
    /\b(build|make|design|create|give|want|need|rocket|ship|vehicle)\b/,
  );
  if (has(text, /\b(moon|lunar)\b/)) {
    const actions: RocketAction[] = [
      { type: "set_destination", value: "moon" },
    ];
    if (building) {
      if (rocket.stages.length < 3)
        actions.push({ type: "add_stage", position: "top" });
      if (!rocket.legs) actions.push({ type: "set_legs", count: 4 });
      if (rocket.payload.kind !== "capsule")
        actions.push({ type: "set_payload", kind: "capsule", crew: 3 });
    }
    emit(
      ctx,
      pick(ctx.rng, [
        "Lunar package loaded. Moon or bust.",
        "Moon configuration ready. Cheese not included.",
      ]),
      ...actions,
    );
  } else if (has(text, /\bmars\b|\bred planet\b|\bmartian\b/)) {
    const actions: RocketAction[] = [
      { type: "set_destination", value: "mars" },
    ];
    if (building) {
      while (
        rocket.stages.length +
          actions.filter((a) => a.type === "add_stage").length <
        3
      )
        actions.push({ type: "add_stage", position: "top" });
      actions.push(
        { type: "set_payload", kind: "habitat", crew: 6 },
        { type: "set_color", target: "accent", value: "#d7261e" },
      );
      if (!rocket.legs) actions.push({ type: "set_legs", count: 4 });
    }
    emit(
      ctx,
      pick(ctx.rng, [
        "Mars configuration ready. Bring snacks. Lots of snacks.",
        "Red planet package installed. It's a long drive.",
      ]),
      ...actions,
    );
  } else if (
    has(text, /\b(the )?sun\b(?! ?glasses)/) &&
    !has(text, /solar panel/)
  ) {
    emit(
      ctx,
...
function boosters(ctx: Context): void {
  ...
  if (
    has(
      text,
      /\b(no|without|remove|lose|ditch|drop|get rid of|delete|zero)\b[a-z ]*\bboosters?\b/,
    )
  ) {
    if (n && n < current) {
      emit(ctx, `Removed ${n} booster${n > 1 ? "s" : ""}.`, {
      ...
      emit(
        ctx,
        current
          ? "All boosters removed. It feels lighter. Lonelier, too."
          : "There were no boosters. I removed them anyway.",
        { type: "remove_boosters" },
      );
    }
    return;
  ...
  if (has(text, /\b(fewer|less)\b[a-z ]*\bboosters?\b/)) {
    ...
    emit(ctx, `Trimmed ${cut} boosters. The accountants cheered.`, {
      type: "remove_boosters",
      count: cut,
    });
    return;
  ...
  const way = has(
    text,
    /\b(way too many|too many|tons of|lots of|loads of|so many|all the|infinite|maximum|max)\b/,
  );
  const mult = multiplier(text);
  let count: number;
  if (n !== null && has(text, /\b(add|more|another|extra|additional|plus)\b/)) {
    count = n;
    emit(ctx, `${n} additional booster${n > 1 ? "s" : ""} installed.`, {
      type: "add_boosters",
      count,
      size,
    });
    ...
    emit(
      ctx,
      count === current
        ? `${count} boosters, as specified.`
        : `${count} booster${count === 1 ? "" : "s"} attached.`,
      { type: "set_booster_count", count, size },
    );
  } else if (way) {
    ...
    emit(
      ctx,
      pick(rng, [
        `${count} boosters attached. I stopped counting at some point.`,
        `Installed ${count} boosters. That's what 'too many' means, right?`,
      ]),
      {
    ...
    emit(
      ctx,
      `Booster count ${mult > 1 ? "multiplied" : "halved"}. Now ${count}.`,
      { type: "set_booster_count", count, size },
    );
  } else if (size && current > 0 && !has(text, /\b(add|more|another)\b/)) {
    emit(ctx, `Boosters resized to ${size}.`, {
      type: "set_booster_count",
      count: current,
      size,
    });
    ...
    emit(
      ctx,
      pick(rng, [
        "Two more boosters strapped on.",
        "Boosters added. Symmetry mostly preserved.",
      ]),
      { type: "add_boosters", count: 2, size },
    );
  }
  const color = colorNear(text, "boosters?");
  if (color)
    emit(ctx, null, { type: "set_color", target: "boosters", value: color });
}
...
function stages(ctx: Context): void {
  ...
  if (has(text, /\bsingle stage\b|\bssto\b|\bone stage\b/)) {
    const removals: RocketAction[] = rocket.stages
      .slice(1)
      .map(() => ({ type: "remove_stage", position: "top" }) as RocketAction);
  ...
  const target =
    n !== null && !has(text, /\b(add|more|another|extra)\b/)
      ? n
      : rocket.stages.length + (n ?? 1);
  const adds = Math.max(0, Math.min(6, target) - rocket.stages.length);
  if (adds === 0) return;
  emit(
    ctx,
    adds > 1
      ? `${adds} stages added. It's a tower now.`
      : "Another stage stacked on top.",
    ...Array.from(
      { length: adds },
      () => ({ type: "add_stage", position: "top" }) as RocketAction,
    ),
  );
}
...
function proportions(ctx: Context): void {
  ...
  if (has(text, /\b(taller|longer|stretch|stretched|higher|tall|elongate)\b/)) {
    ...
    emit(
      ...
      pick(rng, [
        ...
        "Added some vertical ambition.",
      ]),
      { type: "scale", target: "rocket", height: h },
    );
  }
  if (has(text, /\b(shorter|squat|stubby|compact|stumpy)\b/)) {
    emit(ctx, "Shortened. It's cuter now.", {
      type: "scale",
      target: "rocket",
      height: 0.7,
    });
  ...
  const bottomWide =
    has(
      text,
      /\b(bottom|base|first stage|lower)\b[a-z ]*\b(wider|fatter|thicker|bigger|broader)\b/,
    ) ||
    has(text, /\b(wider|fatter|thicker|broader)\b[a-z ]*\b(bottom|base)\b/);
  if (bottomWide) {
    emit(ctx, "Widened the base. Very stable. Very dramatic.", {
      type: "scale",
      target: "id",
      id: ctx.rocket.stages[0].id,
      width: 1.4,
    });
  } else if (
    has(
      text,
      /\b(wider|fatter|thicker|chunkier|girthy|beefier|chonky|chonkier|thick|fat)\b/,
    )
  ) {
    emit(
      ctx,
      pick(rng, ["Beefed it up.", "Chunkier. The chonk is intentional."]),
      { type: "scale", target: "rocket", width: 1.3 },
    );
  }
  if (
    has(
      text,
      /\b(thinner|skinny|skinnier|slimmer|slim|narrow|narrower|pencil)\b/,
    )
  ) {
    emit(ctx, "Slimmed down. Very aerodynamic, allegedly.", {
      type: "scale",
      target: "rocket",
      width: 0.72,
    });
  ...
  const partNouns =
    "boosters?|engines?|fins?|dome|nose|windows?|eyes|ducks?|wings?|thrusters?|nozzles?";
  if (
    has(
      text,
      /\b(enormous|huge|massive|gigantic|giant|colossal|bigger|larger|absolute unit|mega|monstrous|titanic|big)\b/,
    ) &&
    !sizeNear(text, partNouns)
  ) {
    const f = strength > 1 ? strength : 1.6;
    emit(
      ctx,
      pick(rng, [
        "Scaled up. It now has its own weather.",
        "Made it enormous. We'll need a bigger hangar.",
        "Absolute unit mode engaged.",
      ]),
      {
        type: "scale",
        target: "rocket",
  ...
  if (
    has(text, /\b(tiny|smaller|mini|miniature|little|pocket)\b/) &&
    !sizeNear(text, partNouns)
  ) {
    emit(ctx, "Shrunk it. Adorable. Probably still dangerous.", {
      type: "scale",
      target: "rocket",
      height: 0.6,
      width: 0.7,
  ...
  if (
    mult !== 1 &&
    !ctx.actions.some((a) => a.type === "scale") &&
    has(text, /\b(size|big|large|tall)\b/)
  ) {
    emit(ctx, `Scaled by ${mult}×.`, {
      type: "scale",
      target: "rocket",
      height: mult,
      width: Math.sqrt(mult),
    });
  }
}
...
function engines(ctx: Context): void {
  ...
  const mentions = has(
    text,
    /\b(engines?|thrust|power|powerful|nozzles?|motors?|thrusters?)\b/,
  );
  if (!mentions) return;
  const n = numberBefore(text, "engines?|nozzles?|motors?|thrusters?");
  const style = has(text, /aerospike/)
    ? "aerospike"
    : has(text, /trumpet/)
      ? "trumpet"
      : has(text, /flared|flare/)
        ? "flared"
        : has(text, /\bbell\b/)
          ? "bell"
          : undefined;
  const weaker = has(
    text,
    /\b(less|weaker|lower|reduce|gentle|calm|softer)\b[a-z ]*\b(thrust|power|engines?)\b/,
  );
  const stupid = has(
    text,
    /\b(stupidly|absurdly|ridiculously|insanely|obscenely|way more|unreasonably|maximum|max)\b/,
  );
  const bigger = has(
    text,
    /\b(bigger|huge|enormous|massive|giant|large|big|more|powerful|stronger|extra)\b/,
  );
  const current = peakPower(rocket);
  if (weaker) {
    emit(ctx, "Engines throttled down. Everyone can breathe again.", {
      type: "set_engines",
      target: "all",
      power: Math.max(2, current - 3),
    });
    return;
  }
  const action: RocketAction = {
    type: "set_engines",
    target: n !== null ? "core" : "all",
  };
  ...
  const fragment = stupid
    ? pick(rng, [
        "You asked for more thrust. I may have interpreted 'more' aggressively.",
        "Engines upgraded to 'geological event'.",
      ])
    : n !== null
...
function colorNear(text: string, nouns: string): string | undefined {
  ...
  const before = text.match(
    new RegExp(`\\b(${names})\\s+(?:[a-z]+\\s+)?(?:${nouns})\\b`),
  );
  if (before) return before[1];
  const after = text.match(
    new RegExp(`\\b(?:${nouns})\\s+(?:in\\s+|to\\s+|are\\s+)?(${names})\\b`),
  );
  return after?.[1];
}
...
function paint(ctx: Context): void {
  ...
  if (has(text, /\brainbow\b|\bpride\b|\bevery colou?r\b/)) {
    emit(ctx, "Rainbow paint job applied. Visible from orbit.", {
      type: "set_color",
      target: "rainbow",
    });
  }
  const colors = colorsIn(text).filter(
    (c) => c !== "chrome" || !has(text, /\bchrome\b/),
  );
  ...
  if (stripes)
    actions.push({ type: "set_color", target: "secondary", value: stripes });
  if (payload)
    actions.push({ type: "set_color", target: "payload", value: payload });
  const claimed = new Set(
    [
      ...
      payload,
      partColor("boosters?"),
      partColor("engines?|nozzles?|flames?"),
    ].filter(Boolean),
  );
  ...
  const paintVerb =
    has(
      text,
      /\b(paint|painted|colou?r|make it|turn it|all|in|wrap|dye|spray)\b/,
    ) || free.length > 0;
  if (free.length && paintVerb) {
    actions.push({ type: "set_color", target: "all", value: free[0] });
    if (free[1])
      actions.push({ type: "set_color", target: "secondary", value: free[1] });
    if (free[2])
      actions.push({ type: "set_color", target: "accent", value: free[2] });
  }
  if (has(text, /\bchrome\b|\bmirror\b/))
    actions.push(
      { type: "set_finish", value: "chrome" },
      ...(free.length
        ? []
  ...
  else if (has(text, /\b(metallic|metal|brushed|steel|stainless)\b/))
    actions.push({ type: "set_finish", value: "metallic" });
  else if (has(text, /\b(glossy|shiny|shinier|gloss|polished|wet)\b/))
    actions.push({ type: "set_finish", value: "glossy" });
  else if (has(text, /\bmatte\b|\bflat\b|\bstealth\b/))
    actions.push({ type: "set_finish", value: "matte" });
  if (has(text, /\bstealth\b/) && !free.length)
    actions.push({ type: "set_color", target: "all", value: "black" });
  if (has(text, /\bstripe[sd]?\b|\bracing stripes?\b/))
    actions.push({ type: "set_pattern", value: "stripes" });
  else if (has(text, /\bchecker(ed|board)?\b|\bchequered\b/))
    actions.push({ type: "set_pattern", value: "checker" });
  else if (has(text, /\bbands?\b|\bbanded\b/))
    actions.push({ type: "set_pattern", value: "bands" });
  else if (has(text, /\btwo-?tone\b|\bsplit\b|\bhalf and half\b/))
    actions.push({ type: "set_pattern", value: "split" });
  else if (has(text, /\b(plain|solid)\b/))
    actions.push({ type: "set_pattern", value: "solid" });
  if (actions.length) {
    ...
    emit(
      ...
      first
        ? pick(rng, [
            `Painted it ${first}.`,
            `Repainted in ${first}. It looks faster, which is not how physics works.`,
            `${first[0].toUpperCase()}${first.slice(1)} it is.`,
          ])
...
function tops(ctx: Context): void {
  ...
  if (has(text, /\b(dome|glass|observation|bubble|fishbowl|panoram)/)) {
    emit(
      ctx,
      pick(rng, [
        "Observation dome added.",
        "Glass dome installed. Great views, questionable pressure.",
      ]),
      { type: "set_top", kind: "dome" },
    );
  } else if (has(text, /\b(needle|pointy|pointier|sharp|sharper|pointed)\b/)) {
    emit(ctx, "Nose sharpened to a fine point.", {
      type: "set_top",
      kind: "needle",
    });
    ...
    emit(ctx, "Nose removed. Airflow is now everyone's problem.", {
      type: "set_top",
      kind: "none",
    });
...
function payloads(ctx: Context): void {
  const { text } = ctx;
  const crew = numberBefore(
    text,
    "crew|astronauts?|people|passengers?|seats?|tourists?|humans?|cosmonauts?",
  );
  if (has(text, /\b(satellite|probe|telescope)\b/))
    emit(ctx, "Satellite payload loaded.", {
      type: "set_payload",
      kind: "satellite",
    });
  else if (has(text, /\b(cargo|freight|delivery|groceries|pizza)\b/))
    emit(ctx, "Cargo bay installed. Pizza delivery range: extreme.", {
      type: "set_payload",
      kind: "cargo",
    });
  else if (has(text, /\b(habitat|hotel|house|home|apartment|condo|station)\b/))
    emit(ctx, "Habitat module installed. It has a tiny kitchen.", {
      type: "set_payload",
      kind: "habitat",
      crew: crew ?? 6,
    });
  else if (has(text, /\bfairing\b/))
    emit(ctx, "Payload fairing fitted.", {
      type: "set_payload",
      kind: "fairing",
    });
  else if (has(text, /\b(capsule|crewed|crew)\b/) && crew === null)
    emit(ctx, "Crew capsule fitted.", { type: "set_payload", kind: "capsule" });
  if (crew !== null) {
    emit(
      ctx,
      crew > 8
        ? `Seats for ${Math.min(12, crew)}. It's a bus now.`
        : `Crew set to ${crew}.`,
      {
        type: "set_payload",
        crew: Math.min(12, crew),
        ...(ctx.rocket.payload.kind === "capsule" ||
        ctx.rocket.payload.kind === "habitat"
          ? {}
...
function finsAndLegs(ctx: Context): void {
  const { text } = ctx;
  if (has(text, /\bfins?\b/)) {
    if (has(text, /\b(no|without|remove|lose|ditch)\b[a-z ]*\bfins?\b/))
      emit(ctx, "Fins removed. Steering is now vibes-based.", {
        type: "remove_fins",
      });
    else {
      const shape = has(text, /\bgrid\b/)
        ? "grid"
        : has(text, /\bshark\b/)
          ? "shark"
          : has(text, /\bdelta\b/)
      ...
      const size =
        sizeNear(text, "fins?") ??
        (has(text, /\b(bigger|huge|giant|massive)\b/) ? "huge" : undefined);
      emit(
        ctx,
        shape === "grid"
          ? "Grid fins installed. Very waffle."
          : "Fins adjusted.",
        {
          type: "set_fins",
          shape,
          count: n ? Math.min(12, Math.max(2, n)) : undefined,
          size,
        },
      );
    }
  ...
  if (has(text, /\b(legs?|landing|reusable|land itself|landable)\b/)) {
    if (has(text, /\b(no|without|remove)\b[a-z ]*\blegs?\b/))
      emit(ctx, "Landing legs removed. Landing is now optional.", {
        type: "remove_legs",
      });
    else
      emit(ctx, "Landing legs deployed. Reusability: theoretical.", {
        type: "set_legs",
        count: numberBefore(text, "legs?") ?? 4,
      });
  }
}
...
function decorations(ctx: Context): void {
  ...
  for (const [pattern, kind] of DECOR_WORDS) {
    ...
    if (kind === "tank" && !has(text, /\b(external|extra|side|fuel) tanks?\b/))
      continue;
    if (kind === "ring" && has(text, /\bring of boosters\b/)) continue;
    const removing = new RegExp(
      `\\b(no|without|remove|lose|ditch|delete|get rid of)\\b[a-z ]*${pattern.source}`,
    ).test(text);
    if (removing) {
      emit(
        ctx,
        `Removed the ${kind === "googlyEyes" ? "googly eyes. It can no longer see you" : kind}.`,
        { type: "remove_decor", kind },
      );
      continue;
    }
    const n = numberBefore(text, pattern.source.replace(/\\b/g, ""));
    const attach = has(text, /\b(on top|on the top|on the nose|top of)\b/)
      ? "top"
      : has(text, /\b(on the bottom|at the base|bottom)\b/)
        ? "bottom"
        : undefined;
    const color = colorNear(text, pattern.source.replace(/\\b/g, ""));
    const count =
      kind === "googlyEyes"
        ? Math.max(1, Math.ceil((n ?? 2) / 2))
        : Math.min(12, n ?? (kind === "windows" ? 4 : 1));
    emit(ctx, pick(rng, DECOR_LINES[kind]), {
      type: "add_decor",
      kind,
      attach,
      count,
      color,
      size: sizeNear(text, pattern.source.replace(/\\b/g, "")),
    });
  }
}
...
const DECOR_LINES: Record<DecorKind, string[]> = {
  ...
  spikes: [
    "Spikes attached. Nobody touch it.",
    "Spikes added. For no reason whatsoever.",
  ],
  lights: [
    "Lights installed. It's a party now.",
    "LED strips fitted. Now visible from Mars.",
  ],
  wings: ["Wings attached. It is not a plane. Please stop."],
  flag: ["Flag planted. On the rocket. Before arriving."],
  googlyEyes: [
    "Googly eyes installed. It can see you.",
    "Eyes attached. It's watching.",
  ],
  duck: [
    "A rubber duck has been added for morale.",
    "Duck installed. Quack confirmed.",
  ],
  windows: ["Portholes cut. Great for sightseeing."],
  tank: ["External tanks strapped on. Extra fuel, extra chaos."],
  propeller: [
    "Propeller mounted on top. I have no explanation.",
    "Added a propeller. It will not help.",
  ],
};
...
function orientation(ctx: Context): void {
  const { text } = ctx;
  if (has(text, /\bsideways\b|\bhorizontal\b/))
    emit(ctx, "It's sideways now. I have questions.", {
      type: "set_tilt",
      degrees: 90,
    });
  else if (has(text, /\bupside ?down\b|\binverted\b/))
    emit(ctx, "Inverted. The engines are on top. This is fine.", {
      type: "set_tilt",
      degrees: 180,
    });
  else if (has(text, /\b(tilt|lean|leaning|diagonal|crooked|wonky)\b/))
    emit(ctx, "Tilted for style.", { type: "set_tilt", degrees: 25 });
  else if (has(text, /\b(upright|straighten|straight up|vertical)\b/))
    emit(ctx, "Straightened out.", { type: "set_tilt", degrees: 0 });
}
...
function chaos(ctx: Context): void {
  ...
  const intensity = Math.min(
    ...
    multiplier(text) +
      (has(
        text,
        /\b(dumbest|stupidest|most|completely|totally|imaginable|possible|ever)\b/,
      )
        ? 1
        : 0),
  );
  ...
  for (let i = 0; i < 1 + intensity; i++) {
    actions.push({
      type: "add_decor",
      kind: pool[i % pool.length],
      count: randInt(rng, 1, 3),
    });
  }
  if (
    !ctx.actions.some(
      (a) => a.type === "add_boosters" || a.type === "set_booster_count",
    )
  ) {
    actions.push({
      type: "add_boosters",
      count: 2 * intensity + randInt(rng, 1, 3),
      size: pick(rng, ["small", "huge", "medium"] as const),
    });
  }
  actions.push({
    type: "set_engines",
    target: "all",
    power: Math.min(10, 7 + intensity),
  });
  actions.push(
    chance(rng, 0.5)
      ? { type: "set_pattern", value: "checker" }
      : { type: "set_color", target: "rainbow" },
  );
  if (chance(rng, 0.35 * intensity))
    actions.push({ type: "set_tilt", degrees: pick(rng, [12, -18, 25]) });
  if (chance(rng, 0.5))
    actions.push({
      type: "set_top",
      kind: pick(rng, ["dome", "spike", "needle"] as const),
    });
  if (intensity >= 2 && rocket.stages.length < 5)
    actions.push({
      type: "add_stage",
      position: "top",
      height: 6,
...
function sleek(ctx: Context): void {
  const kinds = new Set(
    ctx.rocket.decorativeParts
      .filter((d) => SILLY.includes(d.kind))
      .map((d) => d.kind),
  );
  emit(
    ctx,
    ...
    ...[...kinds].map(
      (kind) => ({ type: "remove_decor", kind }) as RocketAction,
    ),
...
function cheap(ctx: Context): void {
  ...
  emit(
    ctx,
    pick(ctx.rng, [
      "Stripped to the essentials. Duct tape is now load-bearing.",
      "Cheapest rocket possible. The engine is second-hand.",
    ]),
    { type: "remove_boosters" },
    ...rocket.decorativeParts.map(
      (d) => ({ type: "remove_decor", id: d.id }) as RocketAction,
    ),
    ...rocket.stages
      .slice(1)
      .map(() => ({ type: "remove_stage", position: "top" }) as RocketAction),
...
function surprise(ctx: Context): void {
  ...
  const options: (() => RocketAction[])[] = [
    () => [
      {
        type: "add_boosters",
        count: randInt(rng, 2, 6),
        size: pick(rng, ["small", "large", "huge"] as const),
      },
    ],
    () => [
      {
        type: "set_top",
        kind: pick(rng, ["dome", "needle", "spike", "blunt"] as const),
      },
    ],
    () => [
    ...
    () => [
      {
        type: "set_color",
        target: "all",
        value: pick(rng, Object.keys(NAMED_COLORS)),
      },
      {
        type: "set_color",
        target: "secondary",
    ...
    () => [
      {
        type: "set_pattern",
        value: pick(rng, ["stripes", "checker", "bands", "split"] as const),
      },
    ],
    () => [
      {
        type: "scale",
        target: "rocket",
        height: pick(rng, [0.7, 1.4, 1.8]),
        width: pick(rng, [0.8, 1.2, 1.5]),
      },
    ],
    () => [
      {
        type: "set_engines",
        target: "all",
        style: pick(rng, ["aerospike", "flared", "trumpet"] as const),
        power: randInt(rng, 5, 10),
      },
    ],
    ...
    () => [
      {
        type: "set_finish",
        value: pick(rng, ["chrome", "glossy", "metallic"] as const),
      },
    ],
    () => [
      {
        type: "set_fins",
        shape: pick(rng, ["grid", "shark", "delta"] as const),
        size: "large",
      },
    ],
  ];
  ...
  emit(
    ctx,
    pick(rng, [
      "Surprise! I regret nothing.",
      "You said surprise me. I took that personally.",
      "Creative freedom exercised. Liberally.",
    ]),
    ...chosen.flatMap((fn) => fn()),
  );
}
...
export function repairLocally(
  ...
  if (actions.length === 0) {
    actions.push(
      { type: "set_fins", size: "large" },
      {
        type: "set_engines",
        target: "all",
        power: Math.min(8, peakPower(rocket) + 1),
      },
    );
