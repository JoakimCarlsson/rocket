...
import { ThumbnailContext, thumbnailCache as cache, type ThumbnailRequest as Request } from "./thumbnail-context";
...
export function ThumbnailProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ThumbnailContext.Provider value={request}>
      {children}
      <div aria-hidden className="pointer-events-none fixed top-0 -left-[9999px]" style={{ width: WIDTH, height: HEIGHT }}>
        <Canvas dpr={1} gl={{ preserveDrawingBuffer: true, antialias: true }} camera={{ fov: 30, near: 0.5, far: 3000 }} frameloop="always">
          <color attach="background" args={["#0b0c10"]} />
          <ambientLight intensity={0.35} />
          <directionalLight position={[40, 90, 60]} intensity={1.8} color="#fff1e0" />
          <directionalLight position={[-60, 30, -30]} intensity={1.1} color="#7aa7ff" />
          <hemisphereLight args={["#9fb4d8", "#1a1410", 0.6]} />
          <Environment resolution={128} frames={1}>
            <Lightformer form="rect" intensity={3} position={[0, 10, -6]} scale={[16, 3, 1]} />
            <Lightformer form="rect" intensity={1.5} color="#ffd2a8" position={[-8, 3, 0]} rotation={[0, Math.PI / 2, 0]} scale={[10, 5, 1]} />
          </Environment>
...
function Capture({ job, onDone }: { job: Job; onDone: (key: string, url: string) => void }) {
  ...
  useEffect(() => {
    ...
    const extentY = Math.abs(Math.cos(tilt)) * length + Math.abs(Math.sin(tilt)) * layout.width;
    const extentX = Math.abs(Math.sin(tilt)) * length + Math.abs(Math.cos(tilt)) * layout.width;
    const fov = THREE.MathUtils.degToRad(cam.fov);
    const distance = Math.max(extentY / 2 / Math.tan(fov / 2), extentX / 2 / Math.tan(fov / 2) / (WIDTH / HEIGHT)) * 1.35 + layout.width;
    cam.position.set(distance * 0.55, extentY * 0.5 + distance * 0.12, distance * 0.83);
  ...
  return <StaticRocket layout={layout} finish={job.rocket.appearance.finish} tilt={job.rocket.tilt} />;
}
